            ...
            using(MemoryStream ms = new MemoryStream())
            {
                using(StreamWriter sw = new StreamWriter(ms))
                {
                    ...
                    foreach(var pg in pages.Pages)
                    {
                        ...
                        //{
                        //    pg.StudentAnswerSheetImage.Value.Save(ims, imageCodecInfo, encoderParameters);
                        //    sw.Write("{0},{1},{2}", pg.Id,"ImageBytes", BitConverter.ToString(ims.ToArray()));
                        //    sw.WriteLine();
                        //}
